...
class EncodingTools:
    ...
    def get_static_img_path() -> str:
        """获取静态图片文件夹路径"""
        current_file_dir = Path(__file__).parent.parent.parent.parent.parent
        static_img_path = current_file_dir / "static" / "image"
    ...
    def generate_barcode(data: str, barcode_type: str = 'code128', save_to_static: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'filepath': filepath,
                'url': f'/static/image/{os.path.basename(saved_filename)}' if save_to_static else None
            }
    ...
    @staticmethod
    def timestamp_convert(timestamp: Any = None, convert_type: str = 'to_datetime', timestamp_unit: str = 'auto') -> Dict[str, Any]:
        """时间戳转换"""
        try:
    ...
    def base_convert(number: Any, from_base: int = 10, to_base: int = 16) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'from_base': from_base,
                'to_base': to_base,
                'decimal_value': decimal_num if 'decimal_num' in locals() else int(num_str) if from_base == 10 else int(num_str, from_base)
            }
        except Exception as e:
    ...
    @staticmethod
    def url_encode(text: str, encoding: str = 'utf-8', safe: str = '', plus: bool = False) -> Dict[str, Any]:
        """URL编码"""
        try:
            if plus:
                encoded = quote_plus(text, encoding=encoding, safe=safe)
            else:
                encoded = quote(text, encoding=encoding, safe=safe)
            return {
                'result': encoded,
    ...
    @staticmethod
    def url_decode(encoded_text: str, encoding: str = 'utf-8', plus: bool = False) -> Dict[str, Any]:
        """URL解码"""
        try:
            if plus:
                decoded = unquote_plus(encoded_text, encoding=encoding)
            else:
                decoded = unquote(encoded_text, encoding=encoding)
            return {
                'result': decoded,
    ...
    def jwt_decode(token: str, verify: bool = False, secret: str = '') -> Dict[str, Any]:
        ...
        try:
            ...
            return {'error': '无效的JWT令牌，请去掉Bearer 后重试'}
        except Exception as e:
    ...
    @staticmethod
    def base64_decode(encoded_text: str, encoding: str = 'utf-8') -> Dict[str, Any]:
        """Base64解码"""
        try:
            decoded = base64.b64decode(encoded_text.encode(encoding)).decode(encoding)
            return {
                'result': decoded,
                'encoded_length': len(encoded_text),
                'decoded_length': len(decoded)
    ...
    @staticmethod
    def image_to_base64(image_data: bytes, image_format: str = 'png') -> Dict[str, Any]:
        """图片转Base64"""
        try:
    ...
    @staticmethod
    def base64_to_image(base64_data: str, output_format: str = 'png') -> Dict[str, Any]:
        """Base64转图片"""
        try:
            from io import BytesIO
            ...
            import uuid
            timestamp = int(time.time())
            filename = f'image_{timestamp}_{uuid.uuid4().hex[:8]}.{output_format}'
